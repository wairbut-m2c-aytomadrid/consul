...
describe Legislation::Proposal do
  ...
  describe "title length" do
    it "a title length smaller than TITLE_MIN_LENGTH is invalid" do
      proposal = build(:legislation_proposal,
                        title: Faker::Lorem.characters(Legislation::Proposal::TITLE_MIN_LENGTH - 1))
      expect(proposal).not_to be_valid
    end

    it "a title length between TITLE_MIN_LENGTH and TITLE_MAX_LENGTH is valid" do
      title = rand(Legislation::Proposal::TITLE_MIN_LENGTH..Legislation::Proposal::TITLE_MAX_LENGTH)
      proposal = build(:legislation_proposal, title: Faker::Lorem.characters(title))
      expect(proposal).to be_valid
    end

    it "a title length greater than TITLE_MXN_LENGTH is invalid" do
      proposal = build(:legislation_proposal,
                        title: Faker::Lorem.characters(Legislation::Proposal::TITLE_MAX_LENGTH + 1))
      expect(proposal).not_to be_valid
    end
  end
