class Criptografia
    ...
    def encrypt(passkey,str)
        ...
        cipher.encrypt
        cipher.pkcs5_keyivgen encrypt_string(passkey), @salt_bytes.pack('C*').force_encoding('utf-8'),@iterations, 'MD5'
    ...
    def decrypt(passkey,str)
        ...
        uncipher.decrypt
        uncipher.pkcs5_keyivgen encrypt_string(passkey), @salt_bytes.pack('C*').force_encoding('utf-8'),@iterations
        decrypted_str =  uncipher.update(Base64.decode64(str))
        decrypted_str << uncipher.final
    end

    def encrypt_string(passkey)
        "#{passkey}#{Rails.application.secrets.access_key}"
    end
end